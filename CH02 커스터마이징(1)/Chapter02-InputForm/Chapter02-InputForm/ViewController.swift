...
class ViewController: UIViewController {
    ...
    var paramUdate: UISwitch! // 스위치 객체
    ...
    override func viewDidLoad() {
        ...
        let lblUdate = UILabel()
        lblUdate.frame = CGRect(x: 30, y: 150, width: 100, height: 30)
        lblUdate.text = "자동갱신"
        lblUdate.font = UIFont.systemFont(ofSize: 14)
        
        self.view.addSubview(lblUdate)
        ...
        // 스위치 객체를 생성한다.
        self.paramUdate = UISwitch()
        self.paramUdate.frame = CGRect(x: 120, y: 150, width: 50, height: 30)
        
        // 스위치가 ON 되어 있는 상태를 기본값으로 설정한다.
        self.paramUdate.setOn(true, animated: true)
        
        self.view .addSubview(self.paramUdate)
        ...
        self.paramUdate.addTarget(self, action: #selector(presentUpdateValue(_:)), for: .valueChanged)
    ...
    // 전송 버튼과 상호반응 할 액션 메소드
    @objc func submit(_ sender: Any) {
        let rvc = ReadViewController()
        rvc.pEmail = self.paramEmail.text
        rvc.pUpdate = String(self.paramUdate.isOn)
